define('ember-cli-mirage/orm/model', ['exports', 'ember-cli-mirage/orm/associations/belongs-to', 'ember-cli-mirage/orm/associations/has-many', 'ember-cli-mirage/utils/normalize-name', 'ember-cli-mirage/utils/extend', 'ember-cli-mirage/assert', 'ember-cli-mirage/orm/collection', 'ember-cli-mirage/orm/polymorphic-collection', 'lodash/values', 'lodash/compact', 'lodash/assign'], function (exports, _belongsTo, _hasMany, _normalizeName, _extend, _assert, _collection, _polymorphicCollection, _values2, _compact2, _assign2) {
  ...
  */
  class Model {
     ...
     */
    save() {
      let collection = (0, _normalizeName.toInternalCollectionName)(this.modelName);
    ...
    isNew() {
      ...
      if (hasId) {
        let collectionName = (0, _normalizeName.toInternalCollectionName)(this.modelName);
    ...
    */
    _explicitInverseFor(association) {
      ...
      if (candidateInverse && candidate.opts.inverse !== association.key) {
        (0, _assert.default)(false, `You specified an inverse of ${inverse} for ${association.key}, but it does not match ${candidate.modelName} ${candidate.key}'s inverse`);
      }
    ...
    */
    _checkForMultipleExplicitInverses(association) {
      ...
      (0, _assert.default)(matchingExplicitInverses.length <= 1, `The ${this.modelName} model has defined multiple explicit inverse associations for the ${association.ownerModelName}.${association.key} association.`);
    }
    ...
    */
    _implicitInverseFor(association) {
      ...
      return (0, _values2.default)(associations).filter(candidate => candidate.modelName === modelName).reduce((inverse, candidate) => {
        ...
        if (candidateMatches) {
          ...
          (0, _assert.default)(!inverse, `The ${this.modelName} model has multiple possible inverse associations for the ${association.ownerModelName}.${association.key} association.`);
          inverse = candidate;
        }
    ...
    */
    ...
    _setupAttrs(attrs) {
      ...
      Object.keys(attrs).filter(key => {
        let value = attrs[key];
        let isModelOrCollection = value instanceof Model || value instanceof _collection.default || value instanceof _polymorphicCollection.default;
        ...
        (0, _assert.default)(this.associationKeys.indexOf(key) > -1, `You're trying to create a ${this.modelName} model and you passed in a ${modelOrCollection.toString()} under the ${key} key, but you haven't defined that key as an association on your model.`);
      });
    ...
    _validateForeignKeyExistsInDatabase(foreignKeyName, foreignKeys) {
      if (Array.isArray(foreignKeys)) {
        let association = Object.keys(this.hasManyAssociations).map(key => this.hasManyAssociations[key]).filter(association => association.getForeignKey() === foreignKeyName)[0];

        let found;
        if (association.isPolymorphic) {
          found = foreignKeys.map(({ type, id }) => {
            return this.schema.db[(0, _normalizeName.toInternalCollectionName)(type)].find(id);
          });
          found = (0, _compact2.default)(found);
        } else {
          found = this.schema.db[(0, _normalizeName.toInternalCollectionName)(association.modelName)].find(foreignKeys);
        }
        ...
        let association = Object.keys(this.belongsToAssociations).map(key => this.belongsToAssociations[key]).filter(association => association.getForeignKey() === foreignKeyName)[0];

        let found;
        if (association.isPolymorphic) {
          found = this.schema.db[(0, _normalizeName.toInternalCollectionName)(foreignKeys.type)].find(foreignKeys.id);
        } else {
          found = this.schema.db[(0, _normalizeName.toInternalCollectionName)(association.modelName)].find(foreignKeys);
        }

        let foreignKeyLabel = association.isPolymorphic ? `${foreignKeys.type}:${foreignKeys.id}` : foreignKeys;
        (0, _assert.default)(found, `You're instantiating a ${this.modelName} that has a ${foreignKeyName} of ${foreignKeyLabel}, but that record doesn't exist in the database.`);
      }
    }
    ...
    _saveBelongsToAssociations() {
      (0, _values2.default)(this.belongsToAssociations).forEach(association => {
    ...
    _saveHasManyAssociations() {
      (0, _values2.default)(this.hasManyAssociations).forEach(association => {
        this._disassociateFromOldInverses(association);
        this._saveNewAssociates(association);
        this._associateWithNewInverses(association);
      });
      ...
      if (tempAssociation && associateIds) {
        ...
        if (association.isPolymorphic) {
          models = associateIds.map(({ type, id }) => {
            return this.schema[(0, _normalizeName.toCollectionName)(type)].find(id);
          });
        } else {
          // TODO: prob should initialize hasMany fks with []
          models = this.schema[(0, _normalizeName.toCollectionName)(association.modelName)].find(associateIds || []).models;
        }

        models.filter(associate => !associate.isSaving) // filter out models that are already being saved
    ...
    */
    _disassociateFromBelongsToInverse(association) {
      ...
      if (tempAssociation !== undefined && associateId) {
        let associate;
        if (association.isPolymorphic) {
          associate = this.schema[(0, _normalizeName.toCollectionName)(associateId.type)].find(associateId.id);
        } else {
          associate = this.schema[(0, _normalizeName.toCollectionName)(association.modelName)].find(associateId);
        }
    ...
    _disassociateFromDependents() {
      this.schema.dependentAssociationsFor(this.modelName).forEach(association => {
    ...
    */
    ...
    _associateModelWithInverse(model, association) {
      if (model.hasInverseFor(association)) {
        ...
        if (inverse instanceof _belongsTo.default) {
          ...
          this.schema.db[(0, _normalizeName.toInternalCollectionName)(model.modelName)].update(model.id, { [inverseFk]: newId });
        } else {
          let inverseCollection = this.schema.db[(0, _normalizeName.toInternalCollectionName)(model.modelName)];
          let currentIdsForInverse = inverseCollection.find(model.id)[inverse.getForeignKey()] || [];
          let newIdsForInverse = (0, _assign2.default)([], currentIdsForInverse);
    ...
    _updateInDb(attrs) {
      this.attrs = this.schema.db[(0, _normalizeName.toInternalCollectionName)(this.modelName)].update(this.attrs.id, attrs);
    }
